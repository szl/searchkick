...
class IndexTest < Minitest::Test
  ...
  def test_bad_mapping
    ...
    assert_raises(Searchkick::InvalidQueryError) { Product.search("test").response }
  ensure
    Product.reindex
  end
  ...
  def test_missing_index
    assert_raises(Searchkick::MissingIndexError) { Product.search("test", index_name: "not_found").response }
  end
  ...
  def test_unsupported_version
    ...
    Searchkick.client.stub :search, raises_exception do
      assert_raises(Searchkick::UnsupportedVersionError) { Product.search("test").response }
    end
  end

  def test_invalid_query
    assert_raises(Searchkick::InvalidQueryError) { Product.search(body: {query: {boom: true}}).response }
  end
